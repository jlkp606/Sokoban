...
class SokobanPuzzle(search.Problem):
    ...
    def __init__(self, warehouse, goal=None, ignore_box=False, macro=False, allow_taboo_push = False, push_costs = []):
    ...
    def actions(self, state):
        ...
        if (self.macro):
            ...
            #coord of boxes and the direction they can be pushed
            movable_boxes = []

            for box in state.boxes:
                #cells surrounding the box
                #checks which direction the box can be pushed in
                for direction in directions:
                    if (self.can_move_there(state, box, False, direction)):
                        movable_boxes.append((box, direction))

            #each box that can be pushed
            #check if the worker can move to the otherside to push it 
            #eg. if the box needs to be pushed to the right
            #the worker needs to able to be on the Left Side
            for movable_box in movable_boxes:
                if (movable_box[1] == 'Right'):
                    if (can_go_there(state, (movable_box[0][0]-1,movable_box[0][1]))):
                        L.append(movable_box)

                elif (movable_box[1] == 'Left'):
                    if (can_go_there(state, (movable_box[0][0]+1,movable_box[0][1]))):
                        L.append(movable_box)

                elif (movable_box[1] == 'Down'):
                    if (can_go_there(state, (movable_box[0][0],movable_box[0][1]-1))):
                        L.append(movable_box)

                elif (movable_box[1] == 'Up'):
                    if (can_go_there(state, (movable_box[0][0],movable_box[0][1]+1))):
                        L.append(movable_box)
...
def can_go_there(warehouse, dst):
    ...
    problem = SokobanPuzzle(warehouse, goal = dst, ignore_box=True, allow_taboo_push = False)
    #print('start search')
    #t0 = time.time()
    solution = search.astar_graph_search(problem)
    #t1 = time.time()
...
def solve_sokoban_macro(warehouse):
    ...
    problem = SokobanPuzzle(warehouse, ignore_box=False, macro=True, allow_taboo_push = False)
    print('start search')
...
def solve_weighted_sokoban_elem(warehouse, push_costs):
    ...
    @return
        ...
        If a solution exists, return a list of elementary actions that solves
            ...
            If the puzzle is already in a goal state, simply return []
    '''
    problem = SokobanPuzzle(warehouse, ignore_box=False, macro=False, allow_taboo_push = False, push_costs=push_costs)
    print('start search')
    print('Please wait, search in progress')
    t0 = time.time()
    #solution = search.breadth_first_graph_search(problem)
